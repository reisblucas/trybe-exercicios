...
const marketing = (array) => {
  const isUnderAge = array.some(element => element < 18);
  if (isUnderAge) {
...
const verifier = (array) => {
  const reprovados = [];
  array.forEach(element => { // nesse caso é melhor um for loop normal
    if (element.status < 80) {
      const person = {
        nome: element.nome,
        status: element.status,
      }
      reprovados.push(person);
    }
    console.log(reprovados)
    // console.log(`${element.nome}: ${element.status}`);
  });
  console.log(reprovados);
}
