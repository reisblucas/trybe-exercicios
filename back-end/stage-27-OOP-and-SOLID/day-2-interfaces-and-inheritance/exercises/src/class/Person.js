...
var Person = /** @class */ (function () {
    function Person(name, birthDate) {
        ...
        this.name = name;
        var bdTime = new Date(Date.parse(this.birthDate)).getTime();
        ...
        var isFutureDate = bdTime > Date.now();
        if (isFutureDate) {
            throw "A data de nascimento não pode ser uma data no futuro";
        }
        var age = Math.abs(Date.now() - bdTime);
        var ageIsLesserThan120 = Math.floor(age / (1000 * 3600 * 24)) / 365.25;
